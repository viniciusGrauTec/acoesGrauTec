...
public class AcaoGetBaixaMapCarga implements AcaoRotinaJava, ScheduledAction {
	...
	public void doAction(ContextoAcao contexto) throws Exception {
		...
		//	}
			...
			iterarEndpoint(url, token, codEmp, mapaInfIdBaixaOrig,
					mapaInfIdBaixa,
					...
					mapaInfFinanceiroBanco, dataInicio, dataFim);
	...
	public void iterarEndpoint(String url, String token, BigDecimal codemp,
		...
		try {
			...
			while (!atual.isAfter(fim)) {
				...
				String[] response = apiGet2(url + "/financeiro" + "/baixas" 
						 + "?quantidade=0"
	...
	public void iterarEndpoint(String url, String token, BigDecimal codemp,
		...
		try {
			...
			efetuarBaixa(response, url, token, codemp, mapaInfIdBaixaOrig,
					mapaInfIdBaixa,
							...
							if (nufin != null && nufin.compareTo(BigDecimal.ZERO) != 0 && formas_de_pagamento.size() == 1) {
								...
								if (dataEstorno == null) {
									...
									if(dtMinMovConta != null){
										...
										selectsParaInsert.add("SELECT <#NUMUNICO#>, 'Data Minima de Injeção de Saldo Não Localizada Para a Conta: "+codConta+"' , SYSDATE, 'Aviso', "+codemp+", '"+idAluno+"' FROM DUAL");

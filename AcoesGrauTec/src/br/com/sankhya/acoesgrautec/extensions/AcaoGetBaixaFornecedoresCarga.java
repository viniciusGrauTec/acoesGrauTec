...
public class AcaoGetBaixaFornecedoresCarga implements AcaoRotinaJava, ScheduledAction {
	...
	public void doAction(ContextoAcao contexto) throws Exception {
		...
		try{
			...
			iterarEndpoint(url, token, codEmp, mapaInfTipoTituloTaxa, mapaInfBanco,
					...
					mapaInfFinanceiroValor, mapaInfFinanceiroBanco, dataInicio, dataFim);
	...
	public void iterarEndpoint(String url, String token, BigDecimal codemp,
		...
		try {
			...
			while (!atual.isAfter(fim)) {
				...
				//System.out.println("Data atual da iteração: " + dataAtual);
	    ...
	    try {
	        if (response[0].equalsIgnoreCase("200")) {
	            if (response[1] == null || response[1].trim().isEmpty()) {
	                String errorMessage = "Erro: Resposta da API está vazia. Status: " + response[0];
	                System.out.println(errorMessage);
	                selectsParaInsert.add("SELECT <#NUMUNICO#>, '" + errorMessage + "' , SYSDATE, 'Erro', " + codemp + ", '' FROM DUAL");
	                return;
	            }

	            JsonParser parser = new JsonParser();
	            JsonElement jsonElement = parser.parse(response[1]);
	            ...
	            if (jsonElement.isJsonArray()) {
	                ...
	                for (JsonElement element : jsonArray) {
	                    ...
	                    BigDecimal vlrJuros = Optional.ofNullable(jsonObject.get("baixa_juros"))
	                            ...
	                            .map(e -> new BigDecimal(e.getAsString()))
	                    ...
	                    String[] response1 = apiGet2(url + "/financeiro/clientes/titulos-pagar-baixa-forma-pagamento?baixa=" + baixaId, token);
	                    ...
	                    codTipTit = Optional.ofNullable(mapaInfTipoTitulo.get(codemp + "###" + baixaFormaPagamentoId)).orElse(BigDecimal.ZERO);
	                    BigDecimal taxaCartao = Optional.ofNullable(mapaInfTipoTituloTaxa.get(codemp + "###" + baixaFormaPagamentoId)).orElse(BigDecimal.ZERO);
	                                    ...
	                                    if (codTipTit != null && codTipTit.compareTo(BigDecimal.ZERO) != 0) {
	                                        if ("N".equalsIgnoreCase(mapaInfFinanceiroBaixado.get(nufin))) {
	                                            ...
	                                            if (vlrBaixa.compareTo(mapaInfFinanceiroValor.get(nufin)) == 0) {
	                                                ...
	                                                updateFin(codTipTit, nufin, codBanco, codConta, vlrDesconto, vlrJuros, vlrMulta, vlrOutrosAcrescimos, codemp);
	                                            } else {
	                                                System.out.println("Entrou no else do valor");
	                                                updateFinComVlrBaixa(codTipTit, nufin, codBanco, codConta, vlrBaixa, vlrDesconto, vlrJuros, vlrMulta, vlrOutrosAcrescimos, codemp);
	                                            }
	                                        ...
	                                        selectsParaInsert.add("SELECT <#NUMUNICO#>, 'Sem \"de para\" de Tipo de Titulo Configurado Para o Metodo de Pagamento: " + formaDePagamento + "' , SYSDATE, 'Aviso', " + codemp + ", '' FROM DUAL");
	                                    }
	                                } else {
	                                    selectsParaInsert.add("SELECT <#NUMUNICO#>, 'Baixa Para o Titulo " + nufin + " Não Efetuada Pois a Data Minima de Movimentação Bancaria Para a Conta " + codConta + " é Superior a Data de Baixa: " + dataBaixaFormatada + "' , SYSDATE, 'Aviso', " + codemp + ", '' FROM DUAL");
	                                }
	                            } else {
	                ...
	                String errorMessage = "Erro: A resposta da API não é um JSON Array. Resposta: " + response[1];
	                System.out.println(errorMessage);
	                selectsParaInsert.add("SELECT <#NUMUNICO#>, '" + errorMessage + "' , SYSDATE, 'Erro', " + codemp + ", '' FROM DUAL");
	            }
	        } else {
	            String errorMessage = "Erro: Resposta da API com status inválido. Código: " + response[0] + ", Resposta: " + response[1];
	            System.out.println(errorMessage);
	            selectsParaInsert.add("SELECT <#NUMUNICO#>, '" + errorMessage + "' , SYSDATE, 'Erro', " + codemp + ", '' FROM DUAL");
	        }
	    } catch (Exception e) {
	        ...
	        String errorMessage = "Erro no processamento da resposta da API: " + e.getMessage();
	        System.out.println(errorMessage);
	        selectsParaInsert.add("SELECT <#NUMUNICO#>, '" + errorMessage + "' , SYSDATE, 'Erro', " + codemp + ", '' FROM DUAL");
	    }
	}
	...
	public String[] apiGet2(String ur, String token) throws Exception {
		...
		https.setDoInput(true); // Somente entrada, já que é GET
	...
	public List<Object[]> retornarInformacoesTipoTitulo() throws Exception {
		...
		try {
			...
			sql += "		FROM  	AD_TIPTITINTEGRACAO IDEXTERNO IS NOT NULL";
			pstmt = jdbc.getPreparedStatement(sql);
			rs = pstmt.executeQuery();
			while (rs.next()) {
				Object[] ret = new Object[4];

  ...
  implements AcaoRotinaJava, ScheduledAction
 {
	...
	public void iterarEndpoint(Map<String, BigDecimal> mapaInfAlunos,
		...
		try {
			...
			while (!atual.isAfter(fim)) {
				...
				String[] response = apiGet2(url + "/alunos" +
				 "?dataInicial="+dataAtual+" 00:00:00&dataFinal="+dataAtual+" 23:59:59"+
				 "&quantidade=0",
						token);
		...
		try {
			...
			for (JsonElement jsonElement : jsonArray) {
				...
				if (credorCpf != null && credorCidade != null) {
					 ...
					 */
					System.out.println("entrou na validaÁ„o de cadastro");
					if (!credor) {
						 ...
						 */
						System.out.println("Teste validaÁ„o aluno: " + aluno
								+ ": alunoId");
						...
						if (!aluno) {
							...
							insertAluno(credorCadastrado, alunoId, alunoNome,
									...
									credorNome, codEmp, cursoDescricao, turmaId);
							/*
							 * insertLogIntegracao("Aluno j· Cadastro ",
							 * "Aviso", "", alunoNome);
							 */
						} else {
					...
					/*
					 * } else { System.out
					 * .println("Aluno com situaÁ„o diferente do esperado"); }
					 */

				}else{
					selectsParaInsert.add("SELECT <#NUMUNICO#>, 'Credor com informaÁıes invalidas ou nulas', SYSDATE, 'Aviso', "+codEmp+", '"+alunoId+"' FROM DUAL");
					
					//util.inserirLog("Credor com informaÁıes invalidas ou nulas", "Aviso", alunoId, codEmp);
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public BigDecimal insertCredor(String credorNome, String credorCpf,
		...
		try {
			...
			String sqlP = "INSERT INTO TGFPAR(CODPARC, NOMEPARC, RAZAOSOCIAL ,TIPPESSOA, AD_ENDCREDOR, CODBAI, CODCID, "
					+ " CEP,TELEFONE, CGC_CPF, DTCAD, DTALTER, AD_FLAGALUNO) \t\tVALUES(?, ?, ?, ?, ?, NVL((select max(codbai) from tsibai where TRANSLATE( \t\t\t    upper(nomebai), \t\t\t    '·ÈÌÛ˙‚ÍÓÙ˚‡ËÏÚ˘„ıÁ¡…Õ”⁄¬ Œ‘€¿»Ã“Ÿ√’«', \t\t\t    'aeiouaeiouaeiouaocAEIOUAEIOUAEIOUAOC' \t\t\t  ) like TRANSLATE( \t\t\t    upper(?), \t\t\t    '·ÈÌÛ˙‚ÍÓÙ˚‡ËÏÚ˘„ıÁ¡…Õ”⁄¬ Œ‘€¿»Ã“Ÿ√’«', \t\t\t    'aeiouaeiouaeiouaocAEIOUAEIOUAEIOUAOC' \t\t\t  )), 0), (SELECT max(codcid) FROM tsicid WHERE TRANSLATE(              UPPER(descricaocorreio),               '·ÈÌÛ˙‚ÍÓÙ˚‡ËÏÚ˘„ıÁ¡…Õ”⁄¬ Œ‘€¿»Ã“Ÿ√’«',               'aeiouaeiouaeiouaocAEIOUAEIOUAEIOUAOC')               LIKE TRANSLATE(UPPER(?),               '·ÈÌÛ˙‚ÍÓÙ˚‡ËÏÚ˘„ıÁ¡…Õ”⁄¬ Œ‘€¿»Ã“Ÿ√’«',               'aeiouaeiouaeiouaocAEIOUAEIOUAEIOUAOC')               OR SUBSTR(UPPER(descricaocorreio),               1, INSTR(UPPER(descricaocorreio), ' ') - 1)               LIKE TRANSLATE(UPPER(?),               '·ÈÌÛ˙‚ÍÓÙ˚‡ËÏÚ˘„ıÁ¡…Õ”⁄¬ Œ‘€¿»Ã“Ÿ√’«',               'aeiouaeiouaeiouaocAEIOUAEIOUAEIOUAOC')),  ?, ?, ?, SYSDATE, SYSDATE, 'S')";
	...
	public boolean validarCadastroCurso(String curso, String credorNome,
		...
		try {
			...
			String sqlNota = "SELECT COUNT(0) AS COUNT FROM TSICUS  WHERE TRANSLATE(UPPER (DESCRCENCUS), '·ÈÌÛ˙‚ÍÓÙ˚‡ËÏÚ˘„ıÁ¡…Õ”⁄¬ Œ‘€¿»Ã“Ÿ√’«', 'aeiouaeiouaeiouaocAEIOUAEIOUAEIOUAOC') LIKE TRANSLATE (UPPER ('%"
					+ curso.trim()
					+ "%'), "
					+ "'·ÈÌÛ˙‚ÍÓÙ˚‡ËÏÚ˘„ıÁ¡…Õ”⁄¬ Œ‘€¿»Ã“Ÿ√’«', 'aeiouaeiouaeiouaocAEIOUAEIOUAEIOUAOC')";
	...
	public boolean validarCadastroCursoProj(String curso, String credorNome,
		...
		try {
			...
			String sqlNota = "SELECT COUNT(0) AS COUNT FROM TCSPRJ  WHERE TRANSLATE(UPPER (IDENTIFICACAO), '·ÈÌÛ˙‚ÍÓÙ˚‡ËÏÚ˘„ıÁ¡…Õ”⁄¬ Œ‘€¿»Ã“Ÿ√’«', 'aeiouaeiouaeiouaocAEIOUAEIOUAEIOUAOC') LIKE TRANSLATE (UPPER ('"
					+ curso.trim()
					+ "'), "
					+ "'·ÈÌÛ˙‚ÍÓÙ˚‡ËÏÚ˘„ıÁ¡…Õ”⁄¬ Œ‘€¿»Ã“Ÿ√’«', 'aeiouaeiouaeiouaocAEIOUAEIOUAEIOUAOC') AND CODPROJPAI = (SELECT MAX(CODPROJ) FROM TCSPRJ WHERE CODEMP = "
					+ codEmp + ")";
	...
	public boolean validarCadastroTurma(String turma, String credorNome,
		...
		try {
			...
			String sqlNota = "SELECT COUNT(0) AS COUNT FROM TCSPRJ  WHERE TRANSLATE(UPPER (IDENTIFICACAO), '·ÈÌÛ˙‚ÍÓÙ˚‡ËÏÚ˘„ıÁ¡…Õ”⁄¬ Œ‘€¿»Ã“Ÿ√’«', 'aeiouaeiouaeiouaocAEIOUAEIOUAEIOUAOC') LIKE TRANSLATE (UPPER ('%"
					...
					turma.trim()
					+ "%'), "
					+ "'·ÈÌÛ˙‚ÍÓÙ˚‡ËÏÚ˘„ıÁ¡…Õ”⁄¬ Œ‘€¿»Ã“Ÿ√’«', 'aeiouaeiouaeiouaocAEIOUAEIOUAEIOUAOC') AND CODPROJPAI = (SELECT CODPROJ FROM TCSPRJ WHERE CODPROJPAI = (SELECT MAX(CODPROJ) FROM TCSPRJ WHERE CODEMP = "
					+ codEmp + ") AND TRANSLATE(UPPER (IDENTIFICACAO), '·ÈÌÛ˙‚ÍÓÙ˚‡ËÏÚ˘„ıÁ¡…Õ”⁄¬ Œ‘€¿»Ã“Ÿ√’«', 'aeiouaeiouaeiouaocAEIOUAEIOUAEIOUAOC') LIKE TRANSLATE (UPPER ('"
					+
					cursoNome.trim()
					+ "'), "
					+ "'·ÈÌÛ˙‚ÍÓÙ˚‡ËÏÚ˘„ıÁ¡…Õ”⁄¬ Œ‘€¿»Ã“Ÿ√’«', 'aeiouaeiouaeiouaocAEIOUAEIOUAEIOUAOC'))";
	...
	public void updateCurso(String cursoDescricao, String cursoId,
		...
		try {
			...
			String sqlUpdate = "UPDATE TSICUS SET AD_IDEXTERNO = '"
					+ cursoId
					+ "' WHERE CODCENCUS = (SELECT CODCENCUS FROM TSICUS  WHERE TRANSLATE(UPPER (DESCRCENCUS), '·ÈÌÛ˙‚ÍÓÙ˚‡ËÏÚ˘„ıÁ¡…Õ”⁄¬ Œ‘€¿»Ã“Ÿ√’«', 'aeiouaeiouaeiouaocAEIOUAEIOUAEIOUAOC') LIKE TRANSLATE (UPPER ('%"
					+ cursoDescricao.trim()
					+ "'),  '·ÈÌÛ˙‚ÍÓÙ˚‡ËÏÚ˘„ıÁ¡…Õ”⁄¬ Œ‘€¿»Ã“Ÿ√’«', 'aeiouaeiouaeiouaocAEIOUAEIOUAEIOUAOC'))";
	...
	public void insertAluno(BigDecimal credotAtual, String alunoId,
		...
		try {
			...
			String sqlP = "INSERT INTO AD_ALUNOS ( CODPARC, ID_EXTERNO, NOME, NOME_SOCIAL, ENDERECO, CEP, BAIRRO, CIDADE, UF, SEXO, DATA_NASCIMENTO, RG, CPF, TELEFONE_CELULAR, TELEFONE_RESIDENCIAL, EMAIL, SITUACAO, SITUACAO_ID, CODEMP, CODCENCUS, TURMA ) "
					+ "\tVALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, (SELECT TO_CHAR(TO_DATE(?, 'yyyy-MM-dd'), 'dd/MM/yyyy') FROM dual), ?, ?, ?, ?, ?, ?, ?, ?, (select codcencus from tsicus where TRANSLATE(upper(descrcencus), '·ÈÌÛ˙‚ÍÓÙ˚‡ËÏÚ˘„ıÁ¡…Õ”⁄¬ Œ‘€¿»Ã“Ÿ√’«', 'aeiouaeiouaeiouaocAEIOUAEIOUAEIOUAOC') like TRANSLATE(upper('"+descrCurso+"'), '·ÈÌÛ˙‚ÍÓÙ˚‡ËÏÚ˘„ıÁ¡…Õ”⁄¬ Œ‘€¿»Ã“Ÿ√’«', 'aeiouaeiouaeiouaocAEIOUAEIOUAEIOUAOC')), ?)";
					//+ "\tVALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, (SELECT TO_CHAR(TO_DATE(?, 'yyyy-MM-dd'), 'dd/MM/yyyy') FROM dual), ?, ?, ?, ?, ?, ?, ?, ?, 10201001, ?)";
			...
			selectsParaInsert.add("SELECT <#NUMUNICO#>, 'Erro cadastrando aluno: " + e.getMessage() +"', SYSDATE, 'Erro', "+codEmp+", '"+alunoId+"' FROM DUAL");
			
			/*util.inserirLog("Erro cadastrando aluno: " + e.getMessage(),
					"Erro", alunoId, codEmp);*/
			e.printStackTrace();
		} finally {
			if (pstmt != null) {
	...
	public boolean validarCadastroEndereco(String endereco, String credorNome,
		...
		try {
			...
			String sqlNota = "SELECT COUNT(0) AS COUNT FROM TSIEND WHERE TRANSLATE(  \t    upper(NOMEEND),   \t    '·ÈÌÛ˙‚ÍÓÙ˚‡ËÏÚ˘„ıÁ¡…Õ”⁄¬ Œ‘€¿»Ã“Ÿ√’«',   \t    'aeiouaeiouaeiouaocAEIOUAEIOUAEIOUAOC'   \t  ) like TRANSLATE(    \t    upper((SELECT LTRIM(SUBSTR(?, INSTR(?, ' ') + 1))  FROM DUAL)),   \t    '·ÈÌÛ˙‚ÍÓÙ˚‡ËÏÚ˘„ıÁ¡…Õ”⁄¬ Œ‘€¿»Ã“Ÿ√’«',   \t    'aeiouaeiouaeiouaocAEIOUAEIOUAEIOUAOC')";
			...
			e.printStackTrace();
			insertLogIntegracao("Erro ao validar se endereÁo j· cadastrado: "
					+ e.getMessage(), "Erro", credorNome, alunoNome);
			throw e;
		} finally {
			if (rs != null) {
	...
	public boolean validarCadastroBairro(String bairro, String credorNome,
		...
		try {
			...
			String sqlNota = "SELECT COUNT(0) AS COUNT FROM TSIBAI WHERE TRANSLATE(  \t    upper(NOMEBAI),   \t    '·ÈÌÛ˙‚ÍÓÙ˚‡ËÏÚ˘„ıÁ¡…Õ”⁄¬ Œ‘€¿»Ã“Ÿ√’«',   \t    'aeiouaeiouaeiouaocAEIOUAEIOUAEIOUAOC'   \t  ) like TRANSLATE(    \t    upper(?),   \t    '·ÈÌÛ˙‚ÍÓÙ˚‡ËÏÚ˘„ıÁ¡…Õ”⁄¬ Œ‘€¿»Ã“Ÿ√’«',   \t    'aeiouaeiouaeiouaocAEIOUAEIOUAEIOUAOC')";
	...
	public BigDecimal insertBairro(String bairro, String credorNome,
		...
		try {
			...
			se.printStackTrace();
			insertLogIntegracao("Erro ao bairro endereÁo: " + se.getMessage(),
					"Erro", credorNome, alunoNome);
		} finally {
			if (pstmt != null) {
				pstmt.close();

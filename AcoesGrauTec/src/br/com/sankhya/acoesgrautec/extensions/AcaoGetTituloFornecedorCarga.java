...
public class AcaoGetTituloFornecedorCarga implements AcaoRotinaJava, ScheduledAction{
	...
	public void doAction(ContextoAcao contexto) throws Exception {
		...
		try {
			...
			iterarEndpoint(tituloAberto.trim(), tipoEmpresa, mapaInfNaturezaEmp, mapaInfCenCusEmp,
					...
					mapaInfParceiros, url, token, codEmp, dataInicio, dataFim);
	...
	public void iterarEndpoint(String tituloAberto,
			...
			BigDecimal codEmp, String dataInicio,
		...
		try {
			...
			while (!atual.isAfter(fim)) {
				...
				if(tituloAberto.equalsIgnoreCase("S")){
					response = apiGet2(url+ "/financeiro/clientes/titulos-pagar?quantidade=0"
							+"&dataInicial="+dataAtual+" 00:00:00"
							+ "&dataFinal="+dataAtual+" 23:59:59"
							+ "&situacao=A"
							,token);
				}else{
					response = apiGet2(url+ "/financeiro/clientes/titulos-pagar?quantidade=0"
				...
				leituraJSON(tipoEmpresa, mapaInfNaturezaEmp, mapaInfCenCusEmp,
						mapaInfFinanceiroBanco, mapaInfFinanceiroBaixado, 
						mapaInfNatureza, mapaInfBanco, mapaInfConta, 
